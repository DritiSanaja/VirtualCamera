...
2. **Filter Application**: Applies selected filter based on keyboard input
3. **Histogram Calculation**: Computes RGB histograms using optimized Numba functions
4. **Visualization**: Overlays histogram and status text on processed frame
5. **Virtual Camera Output**: Sends processed frame to virtual camera

### Face Detection Methods
- **Haar Cascades**: Fast, traditional method for face detection
- **MediaPipe Face Mesh**: Advanced ML model providing 468 facial landmarks
- **Keypoint-based Replacement**: Uses convex hull of keypoints for precise face region masking

### Background Segmentation
- Uses MediaPipe's pre-trained SelfieSegmentation model
- Confidence threshold of 0.6 for person/background classification
- Supports both image replacement and blur effects
...
- **Matplotlib blitting** for real-time plot updates
- **Pre-initialized figures** to avoid recreation overhead
...
✅ Entropy calculation  
✅ Histogram (RGB channels separately)  
✅ Histogram equalization  
✅ Multiple filters (blur, sharpen, Sobel edge detection)  
✅ No identity filters

### Must-have - Something Special
✅ Object detection (face detection)  
✅ Key points detection (468 facial landmarks)  
✅ Image replacement within key points  
✅ Face → Dog/Cat/Emoji replacement  
✅ Image segmentation tasks  
✅ Pre-trained neural network weights (MediaPipe models)

## License
This project is for educational purposes.

## Authors
- Original framework by @droes (2021)
- Extended with additional features for academic requirements

## Troubleshooting

1. **Camera not detected**: Ensure your webcam is connected and not used by other applications
2. **Missing images**: Make sure all required image files are in the project directory
3. **Import errors**: Verify all dependencies are installed using `pip install -r requirements.txt`
4. **Performance issues**: Try reducing camera resolution or frame rate in `run.py`
